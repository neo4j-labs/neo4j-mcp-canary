...
> **⚠️ Known Issue**: Neo4j version **5.26.18** has a bug in APOC that causes the `get-schema` tool to fail. This issue is fixed in version **5.26.19** and above. If you're using 5.26.18, please upgrade to 5.26.19 or later. See [#136](https://github.com/neo4j-labs/neo4j-mcp-canary/issues/136) for details.
...
By default, there are four requests a MCP Client can send without needing authentication when using HTTP(S) transportation. Some integrations (such as AWS AgentCore, AWS Gateway ) rely on this  as an initial health check mechanism
...
If you do not need these, you can enforce authentication requirements for each by using the relevant environmental variable or command line argument
...
- **Minimum TLS Version**: TLS 1.2 (allows TLS 1.3 negotiation)
...
## Questions for testing use of vectors 


Tier 1: Direct semantic search — the LLM should reach for a vector index immediately.

"Find news articles about artificial intelligence disrupting the retail industry."
— Should use the news vector index on Chunk embeddings. The key insight is that the question is about meaning, not a keyword match.
"Are there any articles discussing supply chain problems or logistics disruptions?"
— Same pattern. There may be no node with the text "supply chain problems" verbatim, so vector similarity is the right path.
"What's been written about companies returning employees to the office?"
— This one is interesting because there's already a fewshot example for return-to-office. An LLM might use the fewshot index to find similar prior questions, or go directly to news.

Tier 2: Vector search + graph traversal — requires combining similarity with relationships.

"Find news similar to the latest Chevron article, but only from articles mentioning companies in the same industry."
— Should: find the Chevron article's chunks, use vector similarity to find similar chunks, then filter by checking whether the parent articles mention organizations that share the same IndustryCategory as Chevron.
"Which competitors of Shopify are mentioned in articles with a similar topic to Shopify's coverage?"
— Should: get chunks from Shopify-related articles, vector-search for similar chunks, trace back to articles → mentioned organizations, and filter to those that are HAS_COMPETITOR with Shopify.
"Find people who are CEOs of organizations mentioned in articles about AI, and show how they're connected to each other."
— Vector search for "AI"-related chunks → parent articles → mentioned orgs → HAS_CEO → Person, then shortest path between them.

Tier 3: Cross-model embedding comparison — testing awareness of multiple vector indexes.

"Search for articles about climate change using the Google embeddings rather than OpenAI."
— The LLM should notice that news_google (768-dim) is a separate index from news (1536-dim) and use embedding_google accordingly.
"Compare the top 5 results for 'electric vehicle market growth' across the OpenAI and SBERT embedding indexes. Do they return the same articles?"
— Should run two separate db.index.vector.queryNodes calls against news and news_sbert, then compare results. This is a strong test of whether the LLM understands there are multiple embedding models indexed.

Tier 4: Fewshot index usage — testing the meta-layer.

"I want to ask 'Which organizations have the highest revenue?' — is there an existing Cypher example for something like that?"
— Should search the fewshot vector index to find similar prior questions and return the stored Cypher template.
"Show me previously asked questions similar to 'How are two people connected in the graph?'"
— Again should hit the fewshot index and surface the Emil Eifrem / Michael Hunger shortest-path example.

Tier 5: Negative cases — the LLM should not use vectors.

"List all organizations in the United States with more than 10,000 employees."
— Pure property filtering + graph traversal (Organization → IN_CITY → City → IN_COUNTRY → Country). No vectors needed.
"Who is the CEO of Coinbase?"
— Direct HAS_CEO relationship traversal. Vectors would be wrong here.
"How many articles mention Epic Games?"
— Simple MATCH (a:Article)-[:MENTIONS]->(o:Organization {name: 'Epic Games'}) RETURN count(a). Count aggregation, no similarity involved.